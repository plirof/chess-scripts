...
if (@$_REQUEST["filter"]!="") {
    ...
    //$temp_keywords_pattern='(u12|u99|etc)';
            ...
            if($do_get_age) { 
              echo '<html><head><script src="include_cr_get_age.js"></script>
                <script type="text/javascript" > const do_get_age=true; </script>

               </head><body class="wholebody" >';
              //echo '<script type="text/javascript" > createLink(); </script>';
              //$name=cr_get_age($string,true)."-".$name; //if($debug) echo "<hr>AGE+name=$name";  
...
<hr>
            ...
            <HR><input type="button" class="createlink" value="Upper Case Username - Create link (takes MORE time!)" onclick="grabAges();"   ><hr>
</BODY>
            </HTML>