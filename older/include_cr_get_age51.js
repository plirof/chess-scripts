...
//Scans all ids that contain word "player", split the list in half and pair them
function initialPairing(name_table){
  // https://bobbyhadz.com/blog/javascript-get-all-elements-by-id-starting-with
  ///const elList = document.querySelectorAll(`[id^="age"]`);
  const elList = document.querySelectorAll(`[id^="player"]`);
  //console.log(elList);
  var counter=0;
  var count_u08=0;
  var count_u10=0;
  ...
  elList.forEach(function(el){
    //console.log(el.value);
    name_text=el.defaultValue;
    if(name_text!==null && name_text!="Advertisement" && !name_text.includes("Break")
      && !name_text.includes("(varia")&& !name_text.includes("Break")
      ) {
      
      name_table[counter]=(counter+1)+")"+name_text;
      counter++;
      //console.log(name_text+"--------NOT NULL")
    }
    //console.log("el.innerHTML -NAME SELECTED  "+name_text);
  ...
  //output.innerHTML = result_pairs;//.value;
  output.innerText = result_pairs;//.value;

} // END of function initialPairing(name_table){
  // ################## PAIR FUNCTIONS###################################
