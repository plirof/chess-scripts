...
Changes
-230401 v52c - Results URL is a variable, removed chessbites.com links (dead site)
-220501 v52a - Changed array filter : $result = array_filter(explode('<h2>Starting rank</h2>', $mytext)); 
...
$results_url='https://chess-results.com/tnr732420.aspx?lan=1&flag=30&turdet=YES';
$keywords_pattern_default='(|u12|u99|etc)';

 ?>
 <html><head><script src="include_cr_get_age52.js"></script>
...
if (@$_REQUEST["url"]!="") {
    ...
    $url = $results_url; // "http://chess-results.com/tnr750117.aspx?lan=1&turdet=YES";//https://chess-results.com/tnr633594.aspx?lan=1";
 }
...
if(strlen($temp_keywords_pattern)<3)  { 
    echo'strlen($temp_keywords_pattern)<3 '."<h2>$temp_keywords_pattern</h2>"; $temp_keywords_pattern=$keywords_pattern_default; };
...
echo "<h1>URL = <a href='".$url."' target=_blank >".$url."</a> </h1>";
...
// $result = array_filter(explode('<div class="defaultDialog"', $mytext));//print_r($result);
 //$result = array_filter(explode('<div class="defaultDialog"', $mytext));print_r($result);
 $result = array_filter(explode('<h2>Starting rank</h2>', $mytext));//print_r($result);
///$result = array_filter(explode('>No.<', $mytext));//print_r($result); //TRY THIS Instead of Starting Rank 
//echo $result[1];
//echo $result[2];
//GET St
//$mytext=$result[2];//Ok with Deafult Dialog
$mytext=$result[1]; //<h2>Starting rank</h2>
echo "<hr size=5>";
//echo $mytext;
...
// site dead."<a href=https://www.chessbites.com/Games.aspx?player=".str_replace(array(' ', ']'),"%2C",$name)." target=_blank >2.chessbites|</a> "
// site dead."<a href=chessbites.com-grab.php?url=https://www.chessbites.com/Games.aspx?player=".str_replace(array(' ', ']'),"%2C",$name)." target=_blank >2b.GRABchessbites|</a> "
//------------------            
