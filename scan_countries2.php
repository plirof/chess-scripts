...
Changes
-240204b - Added ELO average check & archived tournament check
-220820 - Find Weekend - Hide old tournaments
-220819 - Initial version

To Do:
-"To reduce the server load" then it's OLD
...
curl_close($ch); 
$mytext=$output;
//echo "<HR size=15>@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@";print_r($mytext);
    if (strpos($mytext, "To reduce the server load") !== false) {return ["OLD","<font color=darkred >OLD-ARCHIVED</font>","",""];} //240204 jon
...
if (preg_match('/Arbiter(.*?)Pairing/', $mytext, $match) == 1) {
   ...
   // $date=extract_text($match[1],$start_text,$end_text);
    ...
    $ratingavg=extract_next_text($plaintext,"Rating-",13);//240204 jon
    $ratingavg=$array[array_search_partial('Rating-', $array)]; $ratingavg=substr($ratingavg,9 ,25);//240204 jon
  ...
  if (preg_match_all($keywords_pattern, $string)) 
        ...
        {	
          ...
          //  if($debug) echo "<h5>$date_only isWeekend:".isWeekend($date_only)." checkIfFutureDate($date_only):".checkIfFutureDate($date_only)."</h5>";
            ...
            #if (strpos($tourdata, "To reduce the server load") !== false) {$end_result.='<FONT COLOR=red>OLD archived - </FONT></TD></tr>';CONTINUE;} //240204 jon
            if (isWeekend($date_only)) $end_result.='<FONT COLOR=GREEN>WEEKEND- </FONT>';
            if ($tourdata[3]>1800) {$end_result.='<FONT COLOR=CYAN>[elo:'.$tourdata[3].']</FONT>'; $tourdata[3]='<FONT COLOR=DARKMAGENTA>[elo:'.$tourdata[3].']</FONT>';  };

            $end_result.=''.$tourdata[1].'</td><td>'.$tourdata[0].'</td><td>'.$tourdata[2].'</td><td>Elo:<BR>'.$tourdata[3].'</td>'; // 0:date, 1:location,2:time control ,3: Rating-0
